    ...
    if type == 'user':
        ...
        if not user:
            raise fastapi.HTTPException(
                status_code=401, detail='Invalid email or password. Please try entering it again.'
            )
...
async def get_company(company: schemas.Company = fastapi.Depends(services.get_current_company)):
    return company
